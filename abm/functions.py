...
------------
Voter placement
  - sample_from_distribution    : draw one voter position from a
                                  distribution dict returned by environment.py.

Prior generation
  - generate_prior              : π_a ~ Dirichlet(ρ_π · s^0).

Vote counting and reporting
  - countVoteIntentions         : tally votes and update party.voteIntention.
  - voteShares                  : convert raw vote counts to shares.
  - printElectionResults        : formatted console output for one iteration.
  - getWinner                   : party with the highest vote count.
  - summariseStrategicSwitching : fraction of electors deviating from their
                                  sincere choice.

Coordination outcome measures
  - coordination_measures       : ENP, CENP, Δ-CENP, and cliff statistics
                                  comparing sincere and final strategic outcomes.
"""
