...
Execution flow
--------------
1.  Build K equal zones on [-1, 1]; party kernels at zone midpoints.
2.  Build voter distribution for voter placement
    (uniform or unimodal; see environment.py).
3.  Place N electors by sampling from the voter distribution.
4.  Iteration 0: sincere vote (argmax u_a).
5.  Derive true support from sincere vote shares (not pre-specified).
6.  Generate initial poll signal s^0 via temperature transformation +
    Dirichlet draw.
7.  Generate fixed prior beliefs π_a ~ Dirichlet(ρ_π · s^0), once per
    elector.  Priors do not change across iterations.
8.  Strategic loop:
      a. At t=0 (loop iteration 1): posterior m^0_{a,j} = π_{a,j}.
         At t>0 (loop iteration >1): m^t_{a,j} = α π_{a,j} + (1-α) s^t_j.
      b. All electors compute strategic utilities φ_a(j).
      c. Vote tallies recomputed via chooseCandidate.
      d. Multi-step mode (max_iterations > 1): signal refreshed from
         current vote shares starting at iteration 2.
      e. Loop runs for exactly max_iterations iterations.
         There is no early stopping. Final outcome is history[T_max].

Signal design
-------------
The poll signal uses a temperature transformation of the true support
shares followed by a Dirichlet draw to model polling noise:

    s̃_i = (δ_i + ε)^(1/θ) / Σ_j (δ_j + ε)^(1/θ)
    s   ~ Dirichlet(ρ · s̃)

θ < 1  sharpens the signal → coordination-enhancing.
θ = 1  faithful signal (no distortion).
θ > 1  flattens the signal → fragmentation-enhancing.
ρ      controls precision: higher ρ → less noise.

Belief update
-------------
Priors are fixed at initialisation and do not evolve across iterations:

    π_a ~ Dirichlet(ρ_π · s^0)          (drawn once from initial signal)
...
data for calibrating the meaningful range of mu.  Adds "mu_calibration" key:
...
IMPORTANT: should be run with mu=0 so strategic utilities equal raw
strategic gains (no penalty baked in).
"""
