...
The signal applies a temperature transformation to the true support shares,
then draws from a Dirichlet distribution to model polling noise:

    s̃_i = (δ_i + ε)^(1/θ) / Σ_j (δ_j + ε)^(1/θ)   (temperature transform)
    s   ~ Dirichlet(ρ · s̃)                            (signal draw)

θ < 1  sharpens the distribution: front-runners appear stronger than they
       are, viability gaps are amplified → coordination-enhancing signal.
θ = 1  leaves the distribution unchanged → faithful signal.
θ > 1  flattens the distribution: the race appears more open than it is,
       viability gaps are compressed → fragmentation-enhancing signal.

ρ > 0  controls signal precision.  Higher values of ρ generate signals
       closer to s̃ (less noisy); lower values generate more diffuse signals.

The parameters θ and ρ therefore let us vary the *shape* of the signal
(systematic distortion) and its *precision* (random noise) separately.
"""
