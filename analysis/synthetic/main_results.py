...
def plot_figure1() -> None:
    ...
    df_hm.to_csv(ROOT / "fig1_heatmap_trigger_raw.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(ROOT / "fig1_heatmap_trigger.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_figure2() -> None:
    ...
    agg_sw.to_csv(ROOT / "fig2_trigger_condswitch_raw.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(ROOT / "fig2_trigger_condswitch_c.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_figure3() -> None:
    ...
    fig.text(0.5, -0.04, param_spec,
             ha="center", va="top", fontsize=8.5, color="#555555")
    plt.tight_layout()
    plt.savefig(ROOT / "fig3_trajectory_deltacenp.png", dpi=150, bbox_inches="tight")
    plt.close()
...
def load_empirical(filepath=None) -> pd.DataFrame:
    """
    ...
    """
    path = REPO / "data" / "FR-electoral_data.csv" if filepath is None else Path(filepath)
    df = pd.read_csv(path)
...
def load_model_range(saltelli_ks: tuple = (6, 8, 9)) -> dict:
    ...
    for k in saltelli_ks:
        fname = REPO / "data" / f"saltelli_results_K{k}.csv"
        try:
...
def plot_figure4(
    ...
    plt.tight_layout()
    plt.savefig(ROOT / "fig4_empirical_range_cenp.png", dpi=300, bbox_inches="tight")
    plt.close()
