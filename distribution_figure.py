...
----------------------
Plot a symmetric unimodal centrist electorate on the ideological interval.

The figure shows K equal-width ideological zones with party labels, a
Gaussian density (plus uniform floor) centred at ξ = 0, sampled voter
dots coloured by zone, and annotations for ℓ = 2/K (zone spacing), ξ
(attractor location), and ω (distribution width).

Notation follows the paper's parameter table:
    ξ (xi)   : location of the attractor
    ω (omega) : distribution width  (= c · ℓ)
    ε_F      : uniform floor weight
    ℓ (ell)  : zone length = 2/K

Outputs
-------
    distribution_figure.png
...
if __name__ == "__main__":
    fig = draw_figure()
    out = ROOT / "distribution_figure.png"
