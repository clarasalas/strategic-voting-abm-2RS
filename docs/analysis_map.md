...
Nothing structural was changed as a result. The reruns come first; every entry
below is marked **centralize now**, **document as separate**, or **consolidate
after the reruns**.

The reason for the delay is specific rather than general caution. Several of
...
**The catch.** The function is shared, but the *designs* are not:
...
whichever is kept, one of the two current callers changes behaviour on a
degenerate input.
...
duplication is currently harmless and the risk is drift, not error. Deferred for
the same reason as A1: the two designs consume them in different orders, so the
tidy-looking fix is the dangerous one.
...
Done, in `analysis/empirical/lhs_importance.py`. Selection by exclusion —
...
idiom and a natural `extract_outcomes(res)` helper. Pure refactor with no RNG
involvement, but it touches four scripts whose outputs are *not* being
...
> back out of the object that used it — never recomputed at write time.
...
row builder is worth having; the convention matters more than the code sharing.
...
aggregations, but over different shapes and for different questions — those are
category C. Only the binning helper is worth lifting.
...
per *purpose* within a draw; the sweep needs independent streams per *repeat*.
...
`eps_F`; the empirical designs sweep 4 or 5. This is not an oversight — under
empirical replay the signal is exogenous, so the signal-generating parameters
have nothing to do. Keep separate.
...
| tau conversion | `test_tau_units.py` (the conversion, and that runners apply it) and `test_tau_absolute_output.py` (that both units are recorded) | Adjacent, not overlapping |
...
|---|---|---|
| **A** duplicated model logic | — | A1 LHS function, A2 ENP/CENP, A3 ranges |
| **B** shared infrastructure | B1 predictor schema ✔ | B3 outcome extraction, B4 row builder, B5 binning helper |
| **C** intentionally separate | C1–C4 documented ✔ | — |
