...
Overview, then design concepts, then details, following the ABM convention, so
the model can be reimplemented from the description alone.
...
| Metamorphic properties | `test_decision_rule.py`: relabelling invariance (all 120 permutations) and left-right reflection symmetry, both derived from the equations and then verified rather than assumed |
...
### 2.3 Sensitivity analysis, synthetic (ready)
  Saltelli/Sobol design, its outputs, and the stated caveats. Not rerun, because
  §2.5 records why.
...
1. §3 and §1, neither of which depends on the reruns.
2. §2.1 to §2.3 and §2.6, the verification and the synthetic record.
3. The before-side of §2.5, the change record minus the after numbers.
4. **Reruns.**
