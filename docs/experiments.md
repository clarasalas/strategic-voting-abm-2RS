...
| **—** | Horizon / population validation | synthetic | `protocol_validation.py` | varies |
| **—** | Stochastic-noise decomposition | synthetic | `protocol_posthoc.py` | post-hoc |
...
**Synthetic** answers *which parameters drive coordination*. **Empirical** answers
*does the rule reproduce the 2002/2022 contrast*. They share `core_model/` and
nothing else.
...
> ⚠️ `epsilon` here is the electorate floor weight ε<sub>F</sub>, **not** the
...
**The defining design decision of the empirical mode:** *pattern-oriented
calibration, not parameter recovery.* One behavioural draw is applied to **both**
years; only the environment changes between them. No parameter is fitted.
...
> The importance seed is **42**, not `20020422`. The latter is the *design* seed
> and belongs to the sweep, not to the surrogate.
...
> Quick mode writes to a *separate directory*, not to differently-named files in
> the same one. This is deliberate: it makes the dangerous case unrepresentable
...
| `data/empirical_*`, `data/behavioral_*` | ❌ ignored | raw simulation output — bulky, regenerable |
...
Stages 1–3 are the simulation; 4–8 run no simulation at all.
...
|---|---|
| Replay stage failed | **restart with `--overwrite`** — it has no resume; each invocation rewrites from the start |
| Sweep interrupted | **`--resume`** — never `--overwrite`, which discards every completed draw |
| Target files already exist | run refuses unless `--overwrite` is passed |
| Both flags passed | **the CLI refuses** — they are mutually exclusive |
...
The reasoning is the same as for raw simulation output: figures are bulky,
they regenerate from the scripts, and a stale image is harder to detect than a
stale number. Every quantity a reader needs is in a
...
locally; do not treat anything in `figures/` as authoritative.
...
**Why it was needed:** the τ̂ → τ conversion was missing on the empirical side,
...
Operating instructions: [`local_rerun_runbook.md`](local_rerun_runbook.md).
