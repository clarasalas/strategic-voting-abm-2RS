...
round two, or switch to a compromise candidate who can qualify. When does that
individual incentive aggregate into visible coordination, and when does it
fail?
...
The model covers the first round only. Round two is never simulated. It enters
solely as a *projection* that voters form about who will qualify. There is no
campaign, no turnout decision, no abstention, and no candidate entry or exit.
...
Every voter sees the same signal, and there is no private information channel.
...
series have *n*. That offset is documented and pinned by
...
| Parameter | Role |
|---|---|
| **β** ≥ 0 | ideological sharpness. At β = 0 salience decides alone and ideology drops out. As β → ∞ all mass lands on the nearest contender, recovering the nearest rule. |
...
*K* is the number of candidates, and it differs by year: 15 in 2002, 12 in 2022.
So one shared τ̂ draw maps to a *different* absolute tolerance in each year:
...
That is what τ̂ is for: it stays comparable across party systems of different
size, which is exactly what a two-election comparison needs.
...
> an absolute distance. At τ ≥ 2 every party is a contender for every voter, so
> *C<sub>a</sub> ∩ T<sub>R</sub> = ∅* never holds, the trigger never fires, and
> the strategic module sits inert. That defect affected the pre-2026-08-21
> empirical results, where 40 % of the design sat at τ ≥ 2 outright. The model
> emits a `UserWarning` at τ ≥ 2, and zero occurrences appear in the corrected
> run's 30 simulation logs.

### How outputs record both

Every empirical replay and sweep row carries `tau_hat`, `tau_absolute` and `K`,
so the relation is checkable from the CSV alone with no knowledge of how it was
produced. [`tools/validate_rerun.py`](../tools/validate_rerun.py) enforces it to
1e-12.
...
attachment rules on identical footing, and restricting the contender set would
confound the rule being measured with the size of *C<sub>a</sub>*. This value is
not a swept draw, and it is commented as an exception at
...
> ⚠️ **ε<sub>s</sub> = 1e-12, not 1e-4.** The floor is a numerical guard with no
> smoothing role. Earlier documentation quoted `1e-4`; that value is obsolete.
> The current default is at [`model.py:100`](../core_model/model.py) and
...
The signal sequence is the real poll timeline, with no generation step. Weekly
...
| **ρ<sub>π</sub>** (`rho_pi`) | precision of the voter's prior π<sub>a</sub> ~ Dirichlet(ρ<sub>π</sub>·*s*<sup>0</sup>). Higher values cluster priors tightly on the opening poll. |

The previous posterior is never reused. Inertia is anchored to the fixed prior
π<sub>a</sub> rather than to an evolving belief state. At *t = 0* the posterior
is set to π<sub>a</sub> directly, which avoids double-counting *s*<sup>0</sup>
(that signal already generated π<sub>a</sub>).
...
For exactly this reason the behavioural sweep deliberately excludes
ρ<sub>s</sub> as a placebo dimension. The parameters that stay active in
empirical mode are τ̂, μ, α, ρ<sub>π</sub>, and β under probabilistic
initialization.
...
sincerely. The trigger depends on the projection, and never on the voter's own
...
| **μ** | loyalty weight. At μ = 0 the choice is purely instrumental; large μ pins the voter to *j\**. |

> The cost is normalised by ℓ² = (2/K)² so that μ stays comparable across party
> systems of different size, for the same reason as τ̂.
...
> **ENP(δ) = 1 / Σ<sub>j</sub> δ<sub>j</sub>²**, the effective number of parties
> **CENP(δ) = (K − ENP(δ)) / (K − 1)** ∈ [0, 1], coordination-scaled

ENP is the standard Laakso-Taagepera index: an electorate split evenly across 4
candidates has ENP = 4. CENP rescales it so that 0 means maximal fragmentation
and 1 means full coordination, which makes different *K* comparable.

### ΔCENP: two baselines that must never be mixed

> ⚠️ This is the second most error-prone quantity after τ. The project uses two
> different ΔCENP definitions, deliberately, and they are *not* equal.
...
something else: how far the model's own dynamics moved it from its own starting
point.

The two are intentionally left as separate implementations. A contract test pins
them against drift, and deliberately does not assert they are equal.
...
Top-*k* accuracy is an exact set match. It scores 1.0 only if the simulated
top-*k* set equals the actual top-*k* set, and 0.0 otherwise, with no partial
credit, which is why values of exactly 0 are common.
