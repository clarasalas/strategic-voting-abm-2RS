...
environment.py
--------------
Build the ideological space for the strategic voting ABM.

Entry points
------------
- build_equal_zones        : K equal-length zones on [-1, 1]; party
                             kernels at zone midpoints.
- build_voter_distribution : single Gaussian (or skew-normal) + optional
                             uniform floor.
...
Two electorate types are supported:
    ...
    n_modes = 0   Uniform electorate.
                  Voters are placed uniformly on [-1, 1].

    n_modes = 1   Unimodal electorate.
                  Voters cluster around a single mode with a
                  Gaussian (skewness = 0) or skew-normal (skewness ≠ 0)
                  distribution, plus an optional uniform floor:
...
Diffuse/cohesive    :  adjust width_factor (fraction of zone_length = 2/K).

The floor_weight parameter (ε) ensures a minimum probability of voter
placement anywhere on the spectrum, preventing perfectly symmetric
configurations from mechanically pre-solving coordination and suppressing
strategic switching to near zero.  Recommended: 0.05–0.15 for unimodal.
