...
1. c × tau_hat heatmap for trigger rate
2. Two-panel: trigger rate and conditional switching varying c
...
Fixed baseline parameters (justified by Saltelli analysis):
    ...
    eps    = 0.1   floor weight
    K      = 8     even party system with central structure
    xi     = 0.0   symmetric electorate
    M      = 2     French two-round rule
    Tmax   = 25    convergence ceiling
    N      = 2000  electors

Figure 4 reads from:
    outcome_variables_range.xlsx       empirical election data
    saltelli_results_K{6,8,9}.csv      Saltelli output (model range)
    If Saltelli CSVs are not found, the model range band is omitted.

Run cost estimates (adjust N_REPS if needed):
    Figure 1 (heatmap):    10x10 grid × N_REPS runs
...
def plot_figure4(
        empirical_data: str = "outcome_variables_range.xlsx",
    ...
    Parameters
    ----------
    empirical_data : path to Excel file with columns [year, party, poll, result]
    ...
    # ── Empirical data ────────────────────────────────────────────────────
    df = pd.read_excel(empirical_data)
    df.columns = ["year", "party", "poll", "result"]
    df["year"] = df["year"].astype(int)

    rows = []
    for year, group in df.groupby("year"):
        dc = delta_cenp(
            group["poll"].values.astype(float),
            group["result"].values.astype(float),
        )
        rows.append({"year": year, "delta_cenp": round(dc, 4)})
    emp_df = pd.DataFrame(rows).sort_values("year")

    print("Empirical ΔCENP:")
    print(emp_df.to_string(index=False))

    # ── Model range from Saltelli CSVs ────────────────────────────────────
    model_mean = model_p05 = model_p95 = None
    saltelli_dfs = []
    ...
    for k in saltelli_ks:
        ...
        try:
            ...
            saltelli_dfs.append(tmp)
            print(f"Loaded {fname} ({len(tmp)} rows)")
        except FileNotFoundError:
            print(f"{fname} not found — skipping")

    if saltelli_dfs:
        sal_vals = pd.concat(saltelli_dfs, ignore_index=True)["delta_cenp"].dropna()
        model_mean = float(sal_vals.mean())
        model_p05  = float(sal_vals.quantile(0.05))
        model_p95  = float(sal_vals.quantile(0.95))
        print(f"Model ΔCENP: mean={model_mean:.4f}, "
              f"p05={model_p05:.4f}, p95={model_p95:.4f}")
    else:
