"""
Figure: Contender and Opponent Sets  (v3)
Key changes vs v2:
  - voter a placed near E/F boundary (x = 0.685, just inside F)
  - tau = 0.25: left boundary falls inside zone D, past D's kernel
    → illustrates that only parties whose kernel is within tau are contenders
  - Party kernels shown as discrete tick marks on the bottom edge of each zone
  - C_a / O_a labelled directly above the zone groups (not just in legend)
  - Stronger opponent fade for clearer visual contrast
"""
...
if __name__ == "__main__":
    ...
    fig.savefig("figure_contender_opponent_v3.pdf", bbox_inches="tight", dpi=300)
    fig.savefig("figure_contender_opponent_v3.png", bbox_inches="tight", dpi=300)
