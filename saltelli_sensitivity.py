...
-----------------------
Global (variance-based) sensitivity analysis using the Saltelli sampling
scheme and Sobol indices, implemented via SALib.
...
-----
    ...
    To do a quick test run, set N_SALTELLI = 64 (gives 1152 runs).
"""
...
def _run_analysis(K: int) -> dict:
    ...
    pd.DataFrame(param_values, columns=PROBLEM["names"]).to_csv(
        f"saltelli_samples_K{K}.csv", index=False
    )
    ...
    pd.DataFrame(results_rows).to_csv(f"saltelli_results_K{K}.csv", index=False)
    ...
    pd.DataFrame(sobol_rows).to_csv(f"saltelli_sobol_K{K}.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(f"saltelli_sobol_K{K}.png", dpi=150)
    plt.close()
...
def _plot_cross_k(all_sobol: dict) -> None:
    ...
    for outcome in OUTCOMES:
        ...
        plt.tight_layout()
        plt.savefig(f"saltelli_comparison_{outcome}.png", dpi=150)
        plt.close()
...
def _print_summary() -> None:
    ...
    combined = pd.concat(
        [pd.read_csv(f"saltelli_sobol_K{K}.csv") for K in K_VALUES],
        ignore_index=True,
    )
    combined.to_csv("saltelli_sobol_all.csv", index=False)
